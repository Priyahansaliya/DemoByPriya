...
//  DemoSwiftUIByPriya
//
//  Created by Priya Hansaliya on 13/08/24.
//
...
struct BottomalPlayerView: View {
    @State private var player: AVAudioPlayer?
    @State private var isplaying = false
    @State private var totalTime: TimeInterval = 0.0
    @State private var currentTime: TimeInterval = 0.0
            ...
            VStack {
                VStack {
                    HStack(alignment: .center) {
                        ...
                        VStack{
                            Text("Morning Bulletin").font(.title2).foregroundColor(.white)
                            Text("29 Jul, 2024 08:15 AM").font(.subheadline).foregroundColor(.white)
                        }
                        btn({}, Img: "backward")
                        btn({
                            isplaying ? stopAudio() : playAudio()
                    ...
                    HStack(spacing: 10) {
                        Text(timeString(time: currentTime)).font(.subheadline).foregroundColor(.white)
                        Slider(value: Binding(get: {
                            currentTime
                        }, set: { newValue in
                            audioTime(to: newValue)
                        ...
                        Text(timeString(time: totalTime)).font(.body).foregroundColor(.white)
                    }
                }
                .padding(8)
                .background(.blue)
    ...
    @ViewBuilder
    func btn(_ action: ()->(), Img: String) -> some View {
        Button(action: {}, label: {
            Image(Img)
                .resizable()
                .foregroundColor(.white)
